...
theorem neg_eq_of_add_eq_zero {a b : R} (h : a + b = 0) : -a = b := calc
  ...
  _  = -a + a + b := by rw [← h, add_assoc]
  _  = b          := by rw [neg_add_cancel, zero_add]

theorem eq_neg_of_add_eq_zero {a b : R} (h : a + b = 0) : a = -b := by
  rw [← add_zero (-b), ← h, add_comm, add_neg_cancel_right]
...
theorem self_sub_ℝ (a : ℝ) : a - a = 0 := by
...
theorem inv_inv (a : G) : a⁻¹⁻¹ = a := by
  rw [← mul_one a⁻¹⁻¹, ← inv_mul_cancel a, ← mul_assoc, inv_mul_cancel, one_mul]

theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  rw [← inv_mul_cancel a⁻¹, inv_inv]

theorem mul_one (a : G) : a * 1 = a := by
  rw [← mul_inv_cancel a⁻¹, ← mul_assoc, mul_inv_cancel, one_mul, inv_inv]
